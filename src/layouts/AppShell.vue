...
const pageContext = computed(() => ({
  ...
  '/customers': ['客户中心', '客户资料、联系人与跟进记录'],
  '/products': ['产品目录', '产品规格、价格与可售状态'],
  '/inquiries': ['询盘中心', '客户需求识别与响应推进'],
  '/quotations': ['报价中心', '报价制作、审批与客户反馈'],
  '/orders': ['订单交付', '履约进度与交付风险控制'],
  '/tasks': ['跟进任务', '团队待办与提醒安排'],
  '/notifications': ['消息中心', '业务提醒与系统通知'],
  '/governance': ['企业治理', '成员、角色与业务审计']
}[route.path] || ['NexaFlow 工作台', '企业业务数据已隔离']))
const nav = [
  { to: '/', label: '经营总览', icon: DataAnalysis },
  { to: '/customers', label: '客户中心', icon: User },
  { to: '/products', label: '产品目录', icon: Box },
  { to: '/inquiries', label: '询盘中心', icon: Message },
  { to: '/quotations', label: '报价中心', icon: Document },
  { to: '/orders', label: '订单交付', icon: Collection },
...
<template>
  <div class="app-shell">
    <aside class="side-nav">
      ...
      <div class="nav-foot">
        <RouterLink v-if="['OWNER','ADMIN'].includes(auth.role)" to="/governance" class="nav-link"><el-icon><Setting /></el-icon><span>企业治理</span></RouterLink>
        <RouterLink to="/notifications" class="nav-link"><el-icon><Bell /></el-icon><span>消息中心</span><b v-if="unreadCount">{{ unreadCount > 99 ? '99+' : unreadCount }}</b></RouterLink>
    ...
    <section class="workspace-stage">
      <header class="workspace-bar">
        ...
        <div class="workspace-actions">
          ...
          <RouterLink to="/notifications" class="top-notice" aria-label="查看消息中心"><el-icon><Bell /></el-icon><b v-if="unreadCount">{{ unreadCount > 99 ? '99+' : unreadCount }}</b></RouterLink>
