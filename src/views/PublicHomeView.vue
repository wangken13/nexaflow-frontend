...
const workflow = [
  { label: '汇集询盘', detail: '统一承接客户需求与沟通上下文' },
  { label: '识别机会', detail: '提炼需求、风险与下一步行动' },
  { label: '协同报价', detail: '复用产品信息并推进内部审批' },
  { label: '跟进成交', detail: '明确负责人、时限与客户反馈' },
  { label: '履约交付', detail: '持续跟踪订单进度与交付异常' }
]

const capabilities = [
  { icon: Message, title: '客户上下文', text: '客户档案、联系人、询盘原文和跟进记录汇聚在同一业务视图。' },
  { icon: Tickets, title: '商业协同', text: '报价审批、订单推进、交付风险与责任人沿一条流程持续流转。' },
  { icon: DataAnalysis, title: 'AI 业务辅助', text: '依据企业资料分析需求、生成业务草稿，关键结果始终由人确认。' },
  { icon: Lock, title: '企业治理', text: '租户隔离、角色权限、数据范围、敏感信息保护与完整操作审计。' }
]
</script>
  ...
  <main class="public-site">
    ...
    <section class="public-hero" aria-labelledby="hero-title">
      ...
      <div class="public-hero-layout">
        ...
        <aside class="hero-route" aria-label="业务推进路径">
          <ol>
            <li v-for="item in workflow" :key="item.label">
      ...
      <div class="hero-evidence" aria-label="平台价值">
        <div><strong>客户信息不再散落</strong></div>
        <div><strong>团队行动始终同步</strong></div>
        <div><strong>数据与操作严格受控</strong></div>
      </div>
    </section>
    ...
    <section class="public-close">
      <div><h2><span>让团队从下一条询盘开始</span><span>沿着清晰的路径走向交付</span></h2></div>
      <RouterLink class="public-primary" :to="{ path: '/login', query: { redirect: '/app' } }">
