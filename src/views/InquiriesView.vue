...
const sourceLabels: Record<string, string> = { MANUAL: '人工录入', EMAIL: '企业邮箱', WEBSITE: '网站表单', WHATSAPP: 'WhatsApp', WECHAT_WORK: '企业微信', CUSTOM: '外部系统' }
    ...
    <div class="inquiry-layout">
      ...
      <section class="surface inquiry-detail">
        <template v-if="selected">
          <div class="surface-head"><div><h2>{{ selected.subject }}</h2><p>{{ customerName(selected.customerId) }} · {{ sourceLabels[selected.sourceChannel] || selected.sourceChannel }} · {{ formatDateTime(selected.createdAt) }}</p><small>负责人：{{ selected.ownerId || '待分配' }} · 下一步期限：{{ selected.nextActionDue ? formatDateTime(selected.nextActionDue) : '待设置' }}</small></div><el-select v-if="hasPermission(auth.role, 'inquiry:write')" :model-value="selected.status" style="width:160px" @change="changeStatus"><el-option v-for="item in statusOptions" :key="item.value" :label="item.label" :value="item.value"/></el-select><span v-else class="state-chip">{{ labelOf(inquiryStatusLabels, selected.status) }}</span></div>
          ...
          <div v-if="analysis" class="knowledge-grounding" :class="{ insufficient: !analysis.knowledgeSufficient }"><div><strong>{{ analysis.knowledgeSufficient ? '企业知识依据' : '企业资料不足' }}</strong><small>{{ analysis.knowledgeSufficient ? '以下资料参与了本次分析' : '当前建议仅依据客户原文，不能作为正式价格或交期承诺' }}</small></div><div v-if="analysis.sources.length" class="knowledge-sources"><span v-for="source in analysis.sources" :key="source.id">{{ source.title }}</span></div></div>
