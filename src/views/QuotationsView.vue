...
import { tradeApi, type CustomerView, type ProductView, type QuotationApprovalView, type QuotationItemRequest, type QuotationView } from '../api/trade'
...
const approvals = ref<QuotationApprovalView[]>([])
...
const transitionMap: Record<string, Array<{ value: string; label: string; approval?: boolean }>> = {
  DRAFT: [{ value: 'PENDING_APPROVAL', label: '提交审批' }, { value: 'SENT', label: '直接发送' }],
...
const availableTransitions = computed(() => hasPermission(auth.role, 'quotation:write')
  ? (transitionMap[selected.value?.status || ''] || [])
      .filter(item => item.value !== 'SENT' || !selected.value?.approvalRequired)
      .filter(item => !item.approval || hasPermission(auth.role, 'quotation:approve'))
  : [])
...
async function save() { if (!form.value.customerId || form.value.items.some(item => !item.productName || item.quantity <= 0)) return ElMessage.warning('请选择客户并完善报价明细'); saving.value = true; try { const quote = await tradeApi.createQuotation(form.value); editor.value = false; await load(); await select(quote); ElMessage.success(quote.approvalRequired ? '报价单已创建并自动提交审批' : '报价单已创建') } finally { saving.value = false } }
async function select(item: QuotationView) { [selected.value, approvals.value] = await Promise.all([tradeApi.quotation(item.id), tradeApi.quotationApprovals(item.id)]) }
async function changeStatus(status: string) {
  if (!selected.value) return
  if (['PENDING_APPROVAL', 'APPROVED', 'REJECTED'].includes(status) && ['DRAFT', 'PENDING_APPROVAL'].includes(selected.value.status)) { approvalAction.value = status; approvalComment.value = ''; approvalDialog.value = true; return }
  try { selected.value = await tradeApi.updateQuotationStatus(selected.value.id, status); await load(); ElMessage.success('报价状态已更新') } catch (error) { ElMessage.error(error instanceof Error ? error.message : '状态更新失败') }
}
async function confirmApprovalAction() {
  if (!selected.value) return
  if (approvalAction.value === 'REJECTED' && !approvalComment.value.trim()) return ElMessage.warning('驳回报价必须填写原因')
  saving.value = true
  try {
    selected.value = approvalAction.value === 'PENDING_APPROVAL' ? await tradeApi.submitQuotationApproval(selected.value.id, approvalComment.value) : await tradeApi.decideQuotationApproval(selected.value.id, approvalAction.value === 'APPROVED', approvalComment.value)
    approvals.value = await tradeApi.quotationApprovals(selected.value.id); await load(); approvalDialog.value = false; ElMessage.success('审批操作已记录')
  } catch (error) { ElMessage.error(error instanceof Error ? error.message : '审批操作失败') }
  finally { saving.value = false }
}
...
<template>
  <section class="page-shell quotation-page">
    ...
    <div class="master-detail">
      ...
      <section class="surface detail-pane quote-document">
        <template v-if="selected">
          <div class="quote-title"><div><span>客户报价单</span><h2>{{ selected.quotationNo || '报价单' }}</h2><small class="quote-status">{{ statusLabels[selected.status] || '处理中' }}</small></div><div class="no-print action-row"><el-button v-for="action in availableTransitions" :key="action.value" :type="action.value === 'APPROVED' ? 'primary' : 'default'" @click="changeStatus(action.value)">{{ action.label }}</el-button><el-button :icon="Printer" circle title="打印或导出报价单" @click="printQuote"/></div></div>
          <el-alert v-if="selected.approvalRequired" :title="selected.approvalReason || '该报价需要主管审批'" type="warning" :closable="false" show-icon class="no-print"/>
          <div class="quote-meta"><div><label>客户</label><strong>{{ customerName(selected.customerId) }}</strong></div><div><label>贸易条款</label><strong>{{ tradeTermLabel(selected.tradeTerm) }}<template v-if="selected.destinationPort"> · {{ destinationPortLabel(selected.destinationPort) }}</template></strong></div><div><label>有效期</label><strong>{{ formatDate(selected.validUntil) }}</strong></div><div><label>报价币种</label><strong>{{ currencyLabel(selected.currency) }}</strong></div></div>
          <el-table :data="selected.items" class="quote-table"><el-table-column type="index" label="序号" width="64"/><el-table-column prop="productName" label="产品" min-width="160"/><el-table-column prop="specification" label="规格" min-width="180"/><el-table-column prop="quantity" label="数量" width="90"/><el-table-column label="单价" width="120"><template #default="{ row }">{{ Number(row.unitPrice).toFixed(2) }}</template></el-table-column><el-table-column label="金额" width="130"><template #default="{ row }">{{ Number(row.amount).toFixed(2) }}</template></el-table-column></el-table>
          <div class="quote-total"><span>运费 {{ selected.currency }} {{ Number(selected.freight).toFixed(2) }}</span><strong>总计 {{ selected.currency }} {{ Number(selected.totalAmount).toFixed(2) }}</strong></div><p v-if="selected.notes" class="quote-notes">{{ selected.notes }}</p>
        </template>
        <section v-if="selected && approvals.length" class="approval-history no-print"><h3>审批记录</h3><div v-for="item in approvals" :key="item.id"><span class="state-chip">{{ item.action === 'AUTO_SUBMITTED' ? '规则自动提交' : item.action === 'SUBMITTED' ? '提交审批' : item.action === 'APPROVED' ? '审批通过' : '审批驳回' }}</span><p>{{ item.comment || '未填写审批说明' }}</p><small>{{ item.operatorId }} · {{ formatDate(item.createdAt) }}</small></div></section>
        <div v-else-if="!selected" class="empty-state">选择一份报价单查看完整明细</div>
      </section>
    </div>
    ...
    <el-dialog v-model="approvalDialog" :title="approvalAction === 'PENDING_APPROVAL' ? '提交报价审批' : approvalAction === 'APPROVED' ? '批准报价' : '驳回报价'" width="520px"><el-form label-position="top"><el-form-item :label="approvalAction === 'REJECTED' ? '驳回原因' : '审批说明'"><el-input v-model="approvalComment" type="textarea" :rows="5" maxlength="1000" show-word-limit placeholder="记录价格依据、风险或需要调整的内容"/></el-form-item></el-form><template #footer><el-button @click="approvalDialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="confirmApprovalAction">确认操作</el-button></template></el-dialog>
  </section>
</template>
